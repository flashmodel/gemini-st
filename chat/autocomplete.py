"""
Path-segment autocomplete manager for '@' tag file navigation in GeminiCLI chat view.
...
class AutoComplete:
    ...
    def get_workspace_info(
        window,
        chat_workspace_key: str = "gemini_active_workspace"
    ...
    def get_open_files_completions(
        ...
        for v in window.views():
            ...
            # Case-insensitive prefix filter: match relative path or base filename
            if file_filter and not (
                rel_path.lower().startswith(file_filter.lower())
                or file_name.lower().startswith(file_filter.lower())
            ):
                continue
            ...
            completions.append(sublime.CompletionItem(
                ...
                annotation="📂 open tab",
                completion=rel_path,
                kind=getattr(sublime, "KIND_VARIABLE", getattr(sublime, "KIND_AMBIGUOUS", 0))
            ))
    ...
    def generate_completions(
        ...
        flags = getattr(sublime, "INHIBIT_WORD_COMPLETIONS", 0)
        ...
        if not active_cwd:
            return sublime.CompletionList([], flags=flags) if hasattr(sublime, "CompletionList") else []
        ...
        if not query.dir_part:
            ...
            for d in cwd_dirs:
                completions.append(sublime.CompletionItem(
                    ...
                    annotation="📁 folder",
                    completion=d + "/",
                    kind=getattr(sublime, "KIND_NAMESPACE", getattr(sublime, "KIND_AMBIGUOUS", 0))
                ))
        ...
        if hasattr(sublime, "CompletionList"):
    ...
    def check_cascade_trigger(
        ...
        pos = sel[0].b if hasattr(sel[0], "b") else sel[0].begin()
        if pos <= editable_start:
            return
